import type { SemanticOutput } from "@3xhaust/semantic-contract";
...
export type CodingTaskEvent =
	...
	| { readonly type: "capability.started"; readonly capability: string }
	| {
	...
	| { readonly type: "patch.decision"; readonly patchId: string; readonly approved: boolean }
	| {
	...
	| { readonly type: "session.failed"; readonly sessionId: string; readonly message: string };
...
export interface CodingTaskResult {
	...
	readonly usage: CodingTaskUsage;
	readonly patchId?: string;
	readonly diagnostics?: { readonly success: boolean; readonly command: string; readonly output: string };
}
...
export interface PersistedSemanticResult {
	readonly output: SemanticOutput;
	readonly responseId?: string;
	readonly usage: CodingTaskUsage;
}

export interface DurableCodingTaskCheckpoint {
	readonly version: 1;
	readonly phase:
		| "provider-ready"
		| "provider-settled"
